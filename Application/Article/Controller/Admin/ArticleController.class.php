...
class ArticleController extends AdminController {
    ...
    public function index($cid = 0) {
        if (!$cid) {
            ...
            //获取文档列表
            $article_list = D('Article')->getList($cid, C('ADMIN_PAGE_ROWS'), null,$map);
            $article_table = C('DB_PREFIX').'article_'.strtolower($article_type['name']);

            //分页
            $base_table = C('DB_PREFIX').'article_base';
            $page = new Page(D('Article')->where($map)
            ...
            //文档移动POST地址
            $move_url = U('Article/Admin/Article/move');
...
EOF;
            ...
            $builder->setMetaTitle($category_info['title']) //设置页面标题
                    ...
                    ->addTopButton('recycle', array('model' => $this->model)) //添加回收按钮
    ...
    public function edit($id) {
        //获取文档信息
        $article_info = D('Article')->detail($id);
    ...
    public function recycle() {
        $map['status'] = array('eq', '-1');
        $article_list = D('Article')->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))->where($map)->select();
