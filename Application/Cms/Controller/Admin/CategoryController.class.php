...
class CategoryController extends AdminController {
        ...
        public function model() {
            return D('Default')->moduleName.'_category';
        }
...
EOF;
    ...
    public function index($group = 1) { 
        ...
        foreach ($data_list as &$item) {
            ...
            if ($document_type['system'] === '0') {
                $item['title'] = '<a href="'.U(D('Default')->moduleName.'/Admin/Default/index', array('cid' => $item['id'])).'">'.$item['title'].'</a>';
            }
        }
        ...
        $builder->setMetaTitle('分类列表')  // 设置页面标题
                ...
                ->addRightButton('hide', array('model' => $this->model()))    // 添加隐藏/显示按钮
    ...
    public function add($group = 1) {
        if (IS_POST) {
            ...
            $category_object = D('Category');
            if(C('CURRENT_THEME')){
                $template_list = \Common\Util\File::get_dirs(getcwd().'/Theme/'.C('CURRENT_THEME').'/'.D('Default')->moduleName.'/default');
            } else {
                $template_list = \Common\Util\File::get_dirs(getcwd().'/Application/'.D('Default')->moduleName.'/View/Home/default');
            }
    ...
    public function setStatus($model = CONTROLLER_NAME) {
        ...
        switch ($status) {
            case 'delete' :  // 删除条目
                ...
                $count = D('Default')->where($con)->count();
                if ($count == 0) {
