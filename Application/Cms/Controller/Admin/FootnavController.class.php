 ...
 */
class FootnavController extends AdminController {
    // 要操作的模型
    private $model = 'cms_footnav';
    ...
    public function index() {
        ...
        $builder->setMetaTitle('导航列表') // 设置页面标题
                ...
                ->addTopButton('delete', array('model' => $this->model))  // 添加删除按钮
    ...
    public function add() {
        if (IS_POST) {
            $footnav_object = D('Footnav');
            $data = $footnav_object->create();
            if ($data) {
                $id = $footnav_object->add($data);
                if ($id) {
                ...
                $this->error($footnav_object->getError());
            }
        } else {
            ...
            $builder->setMetaTitle('新增导航')  // 设置页面标题
                    ...
                    ->addFormItem('pid', 'select', '上级链接', '上级链接', select_list_as_tree('Footnav', null, '顶级导航'))
