...
class DefaultController extends AdminController {
    ...
    public function index($cid = 0) {
        if (!$cid) {
            ...
            //获取文档列表
            $article_list = D('Default')->getList($cid, C('ADMIN_PAGE_ROWS'), null,$map);
            $article_table = C('DB_PREFIX').D('Default')->moduleName.'_'.$article_type['name'];

            //分页
            $base_table = C('DB_PREFIX').D('Default')->tableName;
            $page = new Page(D('Default')->where($map)
            ...
            //文档移动POST地址
            $move_url = U(D('Default')->moduleName.'/Admin/Default/move');
...
EOF;
            ...
            $builder->setMetaTitle($category_info['title']) //设置页面标题
                    ...
                    ->addTopButton('resume', array('model' => D('Default')->tableName))  //添加启用按钮
                    ->addTopButton('forbid', array('model' => D('Default')->tableName))  //添加禁用按钮
                    ->addTopButton('recycle', array('model' => D('Default')->tableName)) //添加回收按钮
            ...
            $builder->addTableColumn('create_time', '发布时间', 'time')
                    ...
                    ->addRightButton('forbid', array('model' => D('Default')->tableName))  //添加禁用/启用按钮
                    ->addRightButton('recycle', array('model' => D('Default')->tableName)) //添加回收按钮
    ...
    public function edit($id) {
        //获取文档信息
        $article_info = D('Default')->detail($id);
    ...
    public function recycle() {
        $map['status'] = array('eq', '-1');
        $article_list = D('Default')->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))->where($map)->select();
        $page = new Page(D('Default')->where($map)->count(), C('ADMIN_PAGE_ROWS'));

        //使用Builder快速建立列表页面。
        $builder = new \Common\Builder\ListBuilder();
        $builder->setMetaTitle('回收站') //设置页面标题
                ->addTopButton('delete', array('model' => D('Default')->tableName)) //添加删除按钮
                ->addTopButton('restore', array('model' => D('Default')->tableName)) //添加还原按钮
                ...
                ->setTableDataList($article_list) //数据列表
                ->setTableDataPage($page->show()) //数据列表分页
                ->addRightButton('restore', array('model' => D('Default')->tableName)) //添加还原按钮
                ->addRightButton('delete', array('model' => D('Default')->tableName)) //添加删除按钮
                ->display();
    }
}
