...
class DefaultModel extends Model {
    ...
    public function getList($cid, $limit = 10, $order = null, $map = null) {
        ...
        foreach ($article_list as $key => &$val) {
            $val['href'] = U($this->moduleName.'/Home/Default/detail', array('id' => $val['id']));
        }
        return $article_list;
    }
    ...
    public function detail($id) {
        ...
        // 获取分类及文档模型相关信息
        $category_info = D($this->moduleName.'/Category')->find($info['cid']);
        $doc_type_info = D($this->moduleName.'/Type')->find($category_info['doc_type']);
    ...
    private function getPrevious($info) {
        ...
        if (!$previous) {
            ...
            $previous['href'] = U($this->moduleName.'/Home/Default/detail', array('id' => $previous['id']));
        }
        return $previous;
    }
    ...
    private function getNext($info) {
        ...
        if (!$next) {
            ...
            $next['href'] = U($this->moduleName.'/Home/Default/detail', array('id' => $next['id']));
        }
        return $next;
    }
}
