...
class Cms extends TagLib {
    ...
    public function _category_list($tag, $content){
        ...
        $group  = $tag['group'] ? : 1;
        $parse  = '<?php ';
        $parse .= '$__CATEGORYLIST__ = D("Cms/Category")->getCategoryTree('.$pid.', '.$limit.', '.$group.');';
        $parse .= ' ?>';
    ...
    public function _article_list($tag, $content){
        ...
        $parse .= '$__ARTICLE_LIST__ = D("Cms/Default")->getList('.$cid.', '.$limit.', "'.$order.'", $map);';
        $parse .= ' ?>';
    ...
    public function _new_list($tag, $content){
        ...
        $parse  = '<?php ';
        $parse .= '$map["status"] = array("eq", "1");';
        $parse .= '$__ARTICLE_LIST__ = D("Cms/Default")->getNewList('.$doc_type.', '.$limit.', "'.$order.'", $map);';
        $parse .= ' ?>';
        $parse .= '<volist name="__ARTICLE_LIST__" id="'. $name .'">';
        $parse .= $content;
        $parse .= '</volist>';
    ...
    public function _slider_list($tag, $content){
        ...
        $parse  = '<?php ';
        $parse .= '$map["status"] = array("eq", "1");';
        $parse .= '$__SLIDER_LIST__ = D("Cms/slider")->getList('.$limit.', "'.$order.'", $map);';
        $parse .= ' ?>';
    ...
    public function _notice_list($tag, $content){
        ...
        $parse  = '<?php ';
        $parse .= '$map["status"] = array("eq", "1");';
        $parse .= '$__NOTICE_LIST__ = D("Cms/Notice")->getList('.$limit.', "'.$order.'", $map);';
        $parse .= ' ?>';
    ...
    public function _comment_list($tag, $content){
        ...
        $parse  .= '$__COMMENT_LIST__ = D("Cms/Comment")->getCommentList($map);';
        $parse  .= ' ?>';
